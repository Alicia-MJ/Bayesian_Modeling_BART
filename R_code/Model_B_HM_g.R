...
myinits <- list(
  list(mug = 1.2, sigmag = 0.1),
  list(mug = 1.5, sigmag = 0.2))



# parameters to be monitored:
parameters <- c("beta","mug", "sigmag", "gplus", "omega")


samples_g <- jags(data_jags, inits=myinits, parameters, model.file = "modelo_g.txt", 
...
gplus.b <- samples_g$BUGSoutput$sims.list$gplus[,1]
gplus.g <- samples_g$BUGSoutput$sims.list$gplus[,2]

beta.p  <- samples_g$BUGSoutput$sims.list$beta[,1]

omega.b  <- samples_g$BUGSoutput$sims.list$omega[,1]
omega.g  <- samples_g$BUGSoutput$sims.list$omega[,2]
...
plot (density(beta.p), xlab = expression (beta),
